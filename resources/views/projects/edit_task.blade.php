<x-app-layout>
    <div style="padding: 20px;">
        <h1 style="font-size: 24px; margin-bottom: 20px;">Modifier la tâche</h1>
        <form action="{{ route('tasks.update', ['task' => $task, 'project' => $project]) }}" method="post">
            ...
            <input type="text" id="task_name" name="task_name" value="{{ $task->name }}" required style="width: 100%; padding: 8px; border: 1px solid #ccc; border-radius: 5px; margin-bottom: 20px;">
            ...
            <a href="{{ route('tasks.index') }}" style="color: #3498db; text-decoration: none; margin-left: 10px; cursor: pointer;">Annuler</a>
        </form>

        <form action="{{ route('tasks.addUsers', ['task' => $task, 'project' => $project]) }}" method="post" style="margin-top: 20px;">
            @csrf
            ...
            <select id="user_ids" name="user_ids[]" multiple style="width: 100%; padding: 8px; border: 1px solid #ccc; border-radius: 5px; margin-bottom: 10px;">
                @foreach($project->users as $user) <!-- Utilisez les utilisateurs associés au projet -->
