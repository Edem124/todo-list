            ...
            <ul style="list-style: none; padding: 0;">
                @foreach($project->tasks as $task)
                    <li style="margin-bottom: 20px; border-bottom: 1px solid #eee; padding-bottom: 10px;">
                        ...
                        @else
                            <form action="{{ route('tasks.complete', ['task' => $task, 'project' => $project->id]) }}" method="post" style="display: inline;">
                                @csrf
                                @method('PUT')
                            ...
                            <a href="{{ route('tasks.edit', ['task' => $task, 'project' => $project->id]) }}" style="color: #3498db; margin-left: 10px;">Modifier</a>
                        @endif
                        
                        <form action="{{ route('tasks.delete', ['task' => $task, 'project' => $project->id]) }}" method="post" style="display: inline;">
                            @csrf
                            @method('DELETE')
